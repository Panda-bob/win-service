...
/*********************************************************************   
**  GET the loat error code from system and return the string
*********************************************************************/   
...
wx_service::wx_service()
{
	...
	strcpy(szServiceName, "pandabob_win_Service");
}
wx_service::~wx_service()
{
...
void wx_service::Init(){
	...
	GetModuleFileName(NULL,   &m_lpServicePathName[1],   512);   //get exe path 

	char *pre =  &m_lpServicePathName[1];
	char *p = strrchr( pre, '\\');
	snprintf( logFilePath, p-pre , "%s", pre);
	strncat(  logFilePath, ("\\win_up_service.log"), strlen("\\win_up_service.log"));
...
BOOL wx_service::Install()
{
	...
	hService = ::CreateService(
		hSCM, szServiceName, szServiceName, // szServiceName  "pandabob service "
...
BOOL wx_service::OpenService()
{
	...
	bPause = false;
	hSCM = OpenSCManager(NULL, NULL, SC_MANAGER_ALL_ACCESS);
	if( hSCM)               //   SERVICE MANAGER SUCCESS   
	{   
...
void log(LPCTSTR format, ...)
{
 ...
 /*********************************************************************   
  **   put the error log in system event manager 
  *********************************************************************/   
void  LogEvent(LPCTSTR format, ...)
{
	...
	*p = '\0';
	...
	sprintf(lpszMsg,   TEXT("\n%s   notice message:   %d"),   m_pService->szServiceName,   GetLastError());   
	lpszStrings[0]   =   buf;   
